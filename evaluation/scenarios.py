...
def run_unfiltered_context_scenario(
    ...
    if not full_text:
        return f"[No SEC filing data available for {ticker} {year}]", {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}
...
def run_baseline_scenario(
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": """You are a financial analyst assistant. Your job is to answer questions about SEC filings using only your training knowledge.
...
def run_rag_scenario(
    ...
    # Get token usage from the generation step
    chunks = pipeline.search(query=formatted_question, top_k=10)
    result_with_response = pipeline.vector_store.answer_generator.generate_answer_with_response(
        question=formatted_question,
        chunks=chunks
    )
...
def run_reranked_rag_scenario(
    ...
    if not initial_chunks:
        return "[No documents found to rerank]", [], {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}
    ...
    result, response = pipeline.vector_store.answer_generator.generate_answer_with_response(
        question=formatted_question,
