...
class BalancedChunkSampler:
    """Creates a balanced, stratified sample of chunks with optional min token filter."""
    ...
    def __init__(
        ...
        Args:
            max_per_group: maximum samples to draw per (ticker, year, section, size) group.
            balance_companies: whether to equalise number of chunks per company after grouping.
            min_tokens: if provided, discard any chunk whose token count is below this threshold
                *before* grouping.  This lets you avoid generating QA pairs for ultra-short chunks
                like administrative headings ("none."). Set to e.g. 50.
        """
    ...
    def group_chunks_by_keys(self, chunks: List) -> Dict:
        ...
        for chunk in chunks:
            ...
            # Skip if below min_tokens threshold (if set)
            if self.min_tokens is not None and count_tokens(text) < self.min_tokens:
                continue
...
def generate_qa_pairs(
    ...
    try:
        ...
        print(
            "❌ Required packages not installed. Run: pip install langchain langchain-openai tqdm"
        )
        return

    llm = ChatOpenAI(temperature=0, model="gpt-4o-mini", max_tokens=200, max_retries=6)
...
"""
    ...
    progress_bar = tqdm(
        chunks, desc="🤖 Generating QA pairs (via LangChain)", unit="chunk"
    )
            ...
            try:
                ...
                for qa_pair in qa_data.get("qa_pairs", []):
                    ...
                    if question and answer:
                        # Handle both old ('item') and new ('section') field names
                        section = clean_metadata.get("section") or clean_metadata.get(
                            "item"
                        )

                        qa_entry = {
                            "chunk_id": chunk_id,
                            "human_readable_id": clean_metadata.get(
                                "human_readable_id"
                            ),
...
def main():
    ...
    try:
        ...
        print(
            "Could not import from rag package. Make sure it's installed and in the python path."
        )
        sys.exit(1)
    ...
    section_docs = (
        df_sentences.groupby(["docID", "ticker", "fiscal_year", "section"])
        .agg(sentence=("sentence", " ".join))
        .reset_index()
    )

    # The chunker expects a 'sentence_token_count' column.
    section_docs["sentence_token_count"] = section_docs["sentence"].apply(
        lambda x: len(ENCODER.encode(x))
    )

    # Initialize the chunker
    chunker = SmartChunker(target_tokens=750, hard_ceiling=1000, overlap_tokens=150)
