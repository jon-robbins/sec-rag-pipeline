...
from src.openai_functions.qa_generation import BalancedChunkSampler, generate_qa_pairs

from .chunkers import Chunk

logger = logging.getLogger(__name__)

QA_DATASET_PATH = "data/results/qa_datasets/qa_dataset.json"
...
def load_qa_dataset() -> List[Dict[str, Any]]:
    """Loads the QA dataset from the default path."""
    logger.info("Loading QA dataset from %s", QA_DATASET_PATH)
    try:
        with open(QA_DATASET_PATH, "r") as f:
            return json.load(f)
    except FileNotFoundError:
        logger.error(
            "QA dataset not found at %s. Please generate it first.", QA_DATASET_PATH
        )
        return []
