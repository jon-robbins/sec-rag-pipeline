...
def plot_confidence_intervals(
    ...
    for i, (_, row) in enumerate(metric_data.iterrows()):
        ...
        ax.errorbar(
            ...
            y=i,
            xerr=[
                [row["observed_difference"] - row["ci_lower"]],
            ...
            label=(
                "Significant"
                if i == 0 and row["is_significant"]
                else ("Non-significant" if i == 0 else None)
            ),
        )
    ...
    ax.set_title(
        f'{(1-alpha)*100:.0f}% Confidence Intervals for {metric.replace("_", " ").title()} Differences'
    )
    ax.legend()
...
def plot_difference_distributions(
    ...
    for i, metric in enumerate(metrics):
        try:
            ...
            axes[i].axvline(result["ci_upper"], color="orange", linestyle="--")
            axes[i].axvline(
                0, color="black", linestyle=":", alpha=0.5, label="No difference"
            )
    ...
    plt.suptitle(
        f"Bootstrap Difference Distributions: {method1} vs {method2}",
        fontsize=16,
        y=1.02,
    )
    plt.tight_layout()
...
def plot_pairwise_heatmap(
    ...
    methods = list(
        set(metric_data["method1"].tolist() + metric_data["method2"].tolist())
    )
    methods.sort()
    ...
    for i, method1 in enumerate(methods):
        for j, method2 in enumerate(methods):
            if sig_matrix.loc[method1, method2]:
                ax.add_patch(
                    plt.Rectangle(
                        (j, i), 1, 1, fill=False, edgecolor="black", linewidth=3
                    )
                )

    # Customize plot
    ax.set_title(
        f'Pairwise Differences in {metric.replace("_", " ").title()}\n'
...
def create_comprehensive_bootstrap_report(
    ...
    for metric in metrics:
        ...
        save_path_ci = (
            str(output_path / f"{metric}_confidence_intervals.png")
            if output_dir
            else None
        )
        try:
            ...
            warnings.warn(
                f"Failed to create confidence intervals plot for {metric}: {e}"
            )

        # 2. Pairwise heatmap
        save_path_hm = (
            str(output_path / f"{metric}_pairwise_heatmap.png") if output_dir else None
        )
        try:
    ...
    if len(methods) >= 2:
        ...
        for i in range(len(methods)):
            for j in range(i + 1, len(methods)):
                ...
                save_path_dist = (
                    str(output_path / f"{method1}_vs_{method2}_distributions.png")
                    if output_dir
                    else None
                )
                try:
                    ...
                    warnings.warn(
                        f"Failed to create distribution plot for {method1} vs {method2}: {e}"
                    )
...
def plot_token_distribution(
    df, token_col="num_tokens", log_scale=False, title=None, ylim=None
):
    """
...
def plot_top_sections_by_tokens(
    ...
    sec = df.assign(
        section_id=lambda d: d["section_num"].astype(str) + d["section_letter"]
    ...
    section_totals = (
        sec.groupby("section_and_desc")["sentence_token_count"].sum().nlargest(top_n)
    )
    ...
    # ------------------------------------------------------------------ #
    pivot = top_sec.pivot_table(
        index="section_and_desc",
        columns="ticker",
        values="sentence_token_count",
        aggfunc="sum",
        fill_value=0,
    ).loc[
        section_totals.index[::-1]
    ...
    pivot.plot(
        kind="barh", stacked=True, cmap=cmap, edgecolor="white", alpha=0.85, ax=ax
    )
    ...
    for y, total in enumerate(row_totals):
        ax.text(
            total + pad, y, f"{int(total):,}", va="center", ha="left", fontweight="bold"
        )
...
def plot_section_token_distribution_from_df(df_sections: pd.DataFrame):
    """
    Plots the distribution of token counts for sections.
    """
    fig = px.histogram(
        df_sections,
        x="token_count",
        title="Distribution of Section Token Counts",
