...
class DocumentStore:
    ...
    def get_documents_for_chunking(self) -> pd.DataFrame:
        """Gets document/section-level data properly formatted for the Chunker.

        This method groups sentences by document and section, creating the
        intermediate granularity that the Chunker expects.

        Returns
        -------
        pd.DataFrame
            A DataFrame with columns ['ticker', 'fiscal_year', 'section', 'text']
            where each row represents a document section ready for chunking.
        """
        self._load_data_if_needed()
        if self.df is None:
            return pd.DataFrame()

        # Group sentences by document and section to create proper documents for chunking
        grouped_docs = (
            self.df.groupby(["docID", "ticker", "fiscal_year", "section"])
            .agg(
                {
                    "sentence": " ".join,  # Combine sentences into full text
                    "sentence_token_count": "sum",  # Sum up token counts
                }
            )
            .reset_index()
            .rename(
                columns={"sentence": "text", "sentence_token_count": "total_tokens"}
            )
        )
