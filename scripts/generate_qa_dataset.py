...
def generate_and_evaluate_config(config: Dict, config_num: int, total_configs: int):
    ...
    try:
        ...
        eval_results, temp_dir = evaluator.evaluate_all_scenarios(
            num_questions=50, methods=["rag", "reranked_rag", "ensemble_rerank_rag"]
        )
        ...
        for method in ["rag", "reranked_rag", "ensemble_rerank_rag"]:
            if method in summary:
                ...
                results_row[f"{method}_adj_recall_at_1"] = retrieval.get(
                    "adj_recall_at_1", 0
                )
                results_row[f"{method}_adj_recall_at_3"] = retrieval.get(
                    "adj_recall_at_3", 0
                )
                results_row[f"{method}_adj_recall_at_5"] = retrieval.get(
                    "adj_recall_at_5", 0
                )
                results_row[f"{method}_adj_recall_at_10"] = retrieval.get(
                    "adj_recall_at_10", 0
                )
                ...
                results_row[f"{method}_avg_prompt_tokens"] = tokens.get(
                    "prompt_tokens", 0
                )
                results_row[f"{method}_avg_completion_tokens"] = tokens.get(
                    "completion_tokens", 0
                )
                results_row[f"{method}_avg_total_tokens"] = tokens.get(
                    "total_tokens", 0
                )
    ...
    if all_results:
        ...
        if "ensemble_rerank_rag_mrr" in combined_df.columns:
            combined_df = combined_df.sort_values(
                "ensemble_rerank_rag_mrr", ascending=False
            )

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        combined_filename = (
            RESULTS_DIR / f"chunking_comparison_all_configs_{timestamp}.csv"
        )
